...
export const criarTabelaUsuario = async () => {
  ...
  db.transaction(tx => {
    tx.executeSql(
      `CREATE TABLE IF NOT EXISTS usuario (id INTEGER PRIMARY KEY AUTOINCREMENT, nome VARCHAR, contato VARCHAR, perfil VARCHAR)`,
      [],
      result => {
        return true;
      },
...
export const salvarNovoUsuario = async data => {
  ...
  let sql = `insert into usuario (nome, contato, perfil) values (?,?,?)`;
...
export const deleteTable = async data => {
  ...
  await db.transaction(tx => {
    tx.executeSql(
      ...
      error => {
        console.log('Não foi possivel DELETAR tabela de usuarios: ', error);
        throw error;
      },
      () => {
        db.close();
